...
const Pending = ({name, transaction_id, description, transaction_date, value, payment_proof, setOpacity}) => {
    ...
    return (
        <FadeInSection type="slideRight" className="grid grid-cols-4 lg:grid-cols-[2fr_1fr_1fr_1fr] lg:gap-20 mx-auto">
            <label className="lg:pl-16 pl-4 mt-12 lg:text-lg">{name}</label>
            <button className="flex justify-center hover:text-xl mt-12" onClick={() => setShowPendingApproval(true)}><Eye className="mr-2" />View</button>
            <button className="flex justify-center text-[#14AE5C] hover:text-xl mt-12" onClick={showCustomAlert}>Approve<Check className="ml-2" /></button>
            <button className="flex justify-center text-[#F24822] hover:text-xl mr-4 mt-12" onClick={showRejectAlert}>Reject<X className="ml-2" /></button>
            {showPendingApproval && (
                            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-10">
                                <FadeInSection type="zoom" className="lg:ml-[15%] bg-white lg:w-[50%] w-[95%] lg:min-h-screen overflow-y-auto h-[95%] z-30 relative py-6">
                                    ...
                                    <div className="lg:mx-20 mx-6 lg:mt-8 mt-6">
                                        <div className="grid grid-cols-2 lg:gap-16 lg:my-16 my-8 lg:grid-cols-[3fr_2fr]">
                                            ...
                                            <section>
                                                <label className="lg:text-xl font-semibold">Created At</label>
                                                <p className="mt-4 lg:text-xl text-gray-500">{transaction_id}</p>
                                            </section>
                                        </div>
                                        <div className="grid grid-cols-2 lg:gap-16 lg:my-16 my-8 lg:grid-cols-[3fr_2fr]">
                                            <section>
                                                <label className="lg:text-xl font-semibold">Property Type</label>
                                                <p className="mt-4 lg:text-xl text-gray-500">{description}</p>
                                            </section>
                                            <section>
                                                <label className="lg:text-xl font-semibold">Property Location</label>
                                                <p className="mt-4 lg:text-xl text-gray-500">{transaction_date}</p>
                                            </section>
                                        </div>
                                        <section className="lg:gap-16 lg:mb-16 mb-8">
                                            <label className="lg:text-xl font-semibold">Property Description</label>
                                            <p className="mt-4 lg:text-xl text-gray-500">{transaction_date}</p>
                                        </section>
                                        <div className="grid grid-cols-2 lg:gap-16 lg:grid-cols-[3fr_2fr]">
                                            <label className="lg:text-xl font-semibold flex items-center">Annual Rent<span className="lg:ml-4 ml-2 lg:text-xl font-semibold text-[#015A05]">&#8358;{value}</span></label>
                                            <label className="lg:text-xl font-semibold flex items-center">Service Charges<span className="lg:ml-4 ml-2 lg:text-xl font-semibold text-[#015A05]">&#8358;{value}</span></label>
                                            <label className="lg:text-xl font-semibold flex items-center">Other Charges<span className="lg:ml-4 ml-2 lg:text-xl font-semibold text-[#015A05]">&#8358;{value}</span></label>
                                            <label className="lg:text-xl font-semibold flex items-center">Grand Total<span className="lg:ml-4 ml-2 lg:text-xl font-semibold text-[#015A05]">&#8358;{value}</span></label>
                                        ...
                                        <div className="flex items-center justify-center lg:mt-8 mt-6 lg:mb-14">
                                            ...
                                            <p className="italic">Always view attached properties before approving</p>
                                        </div>
